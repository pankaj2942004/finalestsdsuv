...
const UniversityHomepage: React.FC<UniversityHomepageProps> = ({ language, setCurrentPage }) => {
  ...
  const handleMarqueeItemClick = (itemId: string) => {
    const marqueePageMap: { [key: string]: string } = {
  ...
  return (
    <div className="university-homepage">
      <Marquee language={language} onItemClick={handleMarqueeItemClick} />
      <Header language={language} setCurrentPage={setCurrentPage} />
      <div className="main-content">
        <HeroSection language={language} />
        <LatestUpdates language={language} onViewMore={() => setCurrentPage('all-news')} />
      </div>
