...
const AllNewsPage: React.FC<AllNewsPageProps> = ({ language, onBack }) => {
  const allNews = {
    en: [
      {
  ...
  return (
    <div className="detail-page">
      <div className="detail-header">
        ...
        <h1>{language === 'en' ? 'All News & Announcements' : 'सभी समाचार और घोषणाएं'}</h1>
      </div>

      <div className="detail-content news-full-content">
        {newsData.map((newsItem, index) => (
          <div key={index} className="news-full-item">
            <div className="news-header">
              <span className="news-date">{newsItem.date}</span>
              <span className={`news-label ${newsItem.label.toLowerCase()}`}>{newsItem.label}</span>
            </div>
